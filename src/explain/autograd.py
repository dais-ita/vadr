...
class conv2d(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if (explain.shortcut == "shortcut" and shortcut) or (explain.shortcut == "main" and not shortcut) or explain.shortcut is None:
...
class conv3d(Function):
    ...
    def backward(ctx, grad_output):
        ...
        input, weights, bias = ctx.saved_tensors
        stride, padding, dilation, groups, shortcut = ctx.hparam
        if (explain.shortcut == "shortcut" and shortcut) or (
                explain.shortcut == "main" and not shortcut) or explain.shortcut is None:
            weights = torch.clamp(weights, min=0)
        else:
            # essentially turn off this connection by assigning equal contribution to all neurons
            weights = torch.ones_like(weights)
...
class firstconv2d(Function):
    ...
    def backward(ctx, grad_output):
        ...
        root_out[root_out==0] += explain.epsilon
...
class abconv2d(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if beta:
            ...
            csum_out[csum_out==0] += explain.epsilonlon
            c_grad = grad_output / csum_out
            if not explain.epsilonlon:
                c_grad[csum_out == 0] = 0
...
class batch_norm(Function):
    ...
    def forward(ctx, input, running_mean, running_var, weight=None, bias=None,
        ...
        # Training not currently supported for torchexplain
        # if training:
        #     size = input.size()
        #     #
        #     #
