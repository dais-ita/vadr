...
class Postprocessor(object):
    ...
    def __init__(self, pca_params_npz_path=None):
        ...
        self._pca_matrix = torch.as_tensor(params[VGGishParams.PCA_EIGEN_VECTORS_NAME]).float()
        # Load means into a column vector for easier broadcasting later.
        self._pca_means = torch.as_tensor(
            params[VGGishParams.PCA_MEANS_NAME].reshape(-1, 1)
        ).float()
    ...
    def postprocess(self, embeddings_batch):
        ...
        clipped_embeddings = torch.clamp(
            pca_applied, VGGishParams.QUANTIZE_MIN_VAL, VGGishParams.QUANTIZE_MAX_VAL
        )
        # - convert to 8-bit in range [0.0, 255.0]
        quantized_embeddings = (clipped_embeddings - VGGishParams.QUANTIZE_MIN_VAL) * (
            255.0 / (VGGishParams.QUANTIZE_MAX_VAL - VGGishParams.QUANTIZE_MIN_VAL)
        )
